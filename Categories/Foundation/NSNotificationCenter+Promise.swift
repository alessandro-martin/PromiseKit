...
public class NotificationPromise: Promise<[NSObject: AnyObject]> {
    ...
    private class func go() -> (NotificationPromise, (Notification) -> Void) {
        ...
        promise.parentPromise.then { fulfill(($0 as NSNotification).userInfo ?? [:]) }
        return (promise, promise.parentFulfill)
    }

    private override init(@noescape resolvers: (fulfill: ([NSObject: AnyObject]) -> Void, reject: (ErrorProtocol) -> Void) throws -> Void) {
