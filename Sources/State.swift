...
class UnsealedState<R>: State<R> {
    private let barrier = DispatchQueue(label: "org.promisekit.barrier", attributes: DispatchQueueAttributes.concurrent)
    ...
    */
    ...
    override func get(_ body: (Seal<R>) -> Void) {
        ...
        if !sealed {
            barrier.sync(flags: .barrier, execute: {
                switch (self.seal) {
                case .pending:
                    body(self.seal)
                case .resolved:
    ...
    required init(resolver: inout ((R) -> Void)!) {
        ...
        resolver = { resolution in
            ...
            self.barrier.sync(flags: .barrier, execute: {
